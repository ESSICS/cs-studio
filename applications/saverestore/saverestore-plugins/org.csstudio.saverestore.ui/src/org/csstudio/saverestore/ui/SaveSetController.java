...
/**
 *
 * <code>SaveSetController</code> is the controller part for the save set
 * editor. It provides the logic required by the editor.
 *
...
public class SaveSetController {
	...
	/**
	 * Stores the save set data to the repository. Before triggering the actual
	 * store the user is prompt for a short comment describing the changes in the
	 * save set. This method should never be called on the UI thread.
	 *
	...
	public Optional<SaveSetData> save(final SaveSetData data) {
		...
		if (provider.commentOnSaveMandatory()) {
			...
			if(comment.isPresent()) {
				return saveInternal(provider, data, comment.get());
			}
			else {
				return Optional.empty();
			}
			...
			return saveInternal(provider, data, null);
		}
	}
	
	private Optional<SaveSetData> saveInternal(DataProvider provider, final SaveSetData data, String comment){
		try {
			SaveSetData dd = provider.saveSaveSet(data, comment);
	...
	/**
	 * Set the save set data, which the editor currently displays. The object always
	 * represents the saved instance; therefore this is either the object which was
	 * delivered at the time when the editor was opened, or created when the editor
	 * was last saved. This method is not thread safe.
	 *
	 * @param data the data
	 */
	...
	/**
	 * Returns the saved data of this editor - either the data provided when editor
	 * was opened or data created when editor was saved. This method is not thread
	 * safe.
	 *
